...
public class YRDLocalReceiver extends BroadcastReceiver {
    ...
    public void SetAlarm(Activity app, String title, String msg, Calendar alarmTime, long repeatDelay) {
        SetAlarm(app, title, msg, alarmTime, repeatDelay, 0);
    }
    
    public void SetAlarm(Activity app, String title, String msg, Calendar alarmTime, long repeatDelay, int requestCode) {
        CancelAlarm(app, requestCode);
        ...
        PendingIntent pi = PendingIntent.getBroadcast(cxt, requestCode, intent, 0);
    ...
    public void CancelAlarm(Activity app, int requestCode) {
        YRDLog.i(this.getClass(), "cancelled alarm");
        Context cxt = app.getApplicationContext();
        Intent intent = new Intent(cxt, YRDLocalReceiver.class);
        if(intent != null) {
            PendingIntent sender = PendingIntent.getBroadcast(cxt, requestCode, intent, 0);
            if(sender != null)
            {
